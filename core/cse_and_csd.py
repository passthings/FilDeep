...
class MLPCrossSectionEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.layer = torch.nn.Linear(self.input_length * self.input_dim, self.output_length)
    ...
    def forward(self, x):
        ...
        x = self.layer(x)
        x = self.activation(x)
        x = x.view(-1, self.output_length, self.output_dim, 1)
        return x

class MLPCrossSectionDecoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.input_length = 512
        self.input_dim = 1
        ...
        self.layer = torch.nn.Linear(self.input_length * self.input_dim, self.output_length * self.output_dim)
        self.activation = torch.nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = self.layer(x)
        x = self.activation(x)
        x = x.view(-1, 1, self.output_length, self.output_dim)
        return x
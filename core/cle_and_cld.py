...
class MLPCharacteristicLineEncoder(nn.Module):
    def __init__(
            self,
            dim=3,
            input_length=300,
        ...
        self.seq_length = seq_length
        self.emb_dim = emb_dim
        self.layer = nn.Linear(self.input_length * self.dim, self.emb_dim * self.seq_length)


    def forward(self, x):
        x = x.view(x.size(0), -1)
        x = self.layer(x)
        x = x.view(-1, self.seq_length, self.emb_dim)
        return x

class MLPCharacteristicLineDecoder(nn.Module):
    def __init__(
            self,
            dim=3,
            seq_length=60,
        ...
        self.seq_length = seq_length
        self.emb_dim = emb_dim
        self.layer = nn.Linear(self.emb_dim * self.seq_length, self.output_length * self.dim)


    def forward(self, x):
        x = x.view(x.size(0), -1)
        x = self.layer(x)
        x = x.view(-1, self.dim, self.output_length)
        return x
